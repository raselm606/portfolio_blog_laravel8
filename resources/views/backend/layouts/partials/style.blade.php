<!-- Custom fonts for this template-->
<link href="{{asset('public/backend/vendor/fontawesome-free/css/all.min.css')}}" rel="stylesheet" type="text/css">
<link
    href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i"
    rel="stylesheet">

<!-- Custom styles for this template-->
{{--<link href="{{asset('backend/css/sb-admin-2.min.csss')}}" rel="stylesheet">--}}
<link href="{{asset('public/backend/css/sb-admin-2.css')}}" rel="stylesheet">
<!-- Custom styles for this page -->
<link href="{{asset('public/backend/vendor/datatables/dataTables.bootstrap4.min.css')}}" rel="stylesheet">
<link href="{{asset('public/backend/vendor/toastr/toastr.min.css')}}" rel="stylesheet">
<link href="{{asset('public/backend/vendor/summernote/summernote.min.css')}}" rel="stylesheet">
